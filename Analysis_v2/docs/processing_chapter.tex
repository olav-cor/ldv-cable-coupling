...
estimators of the cable elongation are implemented; their agreement is a
built-in consistency check of the processing.

\subsubsection{Method 1: Spatial Gradient of the Axial Displacement}
...
$\delta x_\ell^{(1)}(t) = \int_\mathrm{gap} \varepsilon(s,t)\,\mathrm{d}s$.

\subsubsection{Method 2: Per-Segment 3-D Arc-Length Elongation}
\label{sec:proc:strain:m2}
% ldv_analysis/strain.py :: strain_3d_arclength
Method 2 estimates the elongation of each segment between two adjacent scan
...
direction, so Method 2 measures the \emph{arc-length} change, which is
...
\begin{equation}
  ...
  \label{eq:deltaxl}
\end{equation}
Method 2 is the primary estimator in this work: it makes no assumption
...
scatter strongly wherever excitation or response is weak. It is shown as the
baseline that the averaged estimators improve upon.
