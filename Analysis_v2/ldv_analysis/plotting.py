...
def plot_fig3_transfer(datasets, f_max=60.0, coh_thresh=0.7,
                       ...
                       figsize=(13, 9), title=None, phase_mode='wrapped'):
    ...
    One overlaid four-panel figure across the given datasets:
        ...
        (b) arg(H_mid(f)) [deg]                     — "midpoint phase"
        (c) |H_η(f)| × 100 %                        — "strain transfer amplitude"
        (d) arg(H_η(f)) [deg]                       — "strain transfer phase"

    phase_mode: 'wrapped' (default) keeps arg(H) in (−180°, 180°] — robust when
    low-coherence bins make the unwrapped phase run away; 'unwrapped' applies
    np.unwrap for a continuous phase curve (Simone Fig. 3 style).
    ...
    for cfg in datasets:
        ...
        # Panel d — strain-transfer phase [deg]
        if phase_mode == 'unwrapped':
            ph_eta = np.rad2deg(np.unwrap(np.angle(tf['H_eta'][band])))
        else:
            ph_eta = np.rad2deg(np.angle(tf['H_eta'][band]))
        ...
        if show_stft and cfg.get('fd_stft') is not None:
            ...
            ax_c.plot(sf, se, marker=marker, ls='none', mfc='none',
                      mec=col, ms=7, mew=1.3, zorder=4)
            pm = np.rad2deg(np.angle(st['H_mid'][sb]))
            pe = np.rad2deg(np.angle(st['H_eta'][sb]))
            if phase_mode == 'unwrapped':
                pm = _align_point_phase(sf, pm, fb,
                        np.rad2deg(np.unwrap(np.angle(tf['H_mid'][band]))))
                pe = _align_point_phase(sf, pe, fb,
                        np.rad2deg(np.unwrap(np.angle(tf['H_eta'][band]))))
...
def plot_estimator_comparison(cfg, which='eta', f_max=300.0, coh_thresh=0.7,
                              phase_mode='wrapped', logy=False,
                              figsize=(12, 8)):
    """Compare the four FRF estimators for one dataset: H1, H2, Hv, direct.

    Three stacked panels sharing the frequency axis:
        (1) |H(f)|            — H1 / H2 / Hv on the Welch grid, plus the raw
                                single-FFT direct estimate (faint, dense grid).
        (2) arg H(f) [deg]    — wrapped by default (phase_mode='unwrapped'
                                switches to np.unwrap on the Welch curves).
        (3) coherence γ²(f)   — with the coh_thresh gate line.

    |H1| ≤ |Hv| ≤ |H2| holds bin-by-bin; the spread between H1 and H2 widens
    exactly where γ² drops, which makes this figure the visual justification
    for the coherence-gated band averages used in the η summaries.
    ...
    est_style = {'h1': dict(color='C0', lw=1.5, label=r'$H_1 = S_{XY}/S_{XX}$'),
                 ...
                 'hv': dict(color='C2', lw=1.2, ls='--',
                            label=r'$H_v$ (total least squares)')}
    for est, style in est_style.items():
        H = tf[f'H_{which}_{est}'][band]
        ax_a.plot(fb, np.abs(H) * scale, zorder=3, **style)
        ph = np.angle(H)
        if phase_mode == 'unwrapped':
            ph = np.unwrap(ph)
        ax_p.plot(fb, np.rad2deg(ph), zorder=3, **{k: v for k, v in style.items()
                                                   if k != 'label'})

    ax_c.plot(fb, coh, color='k', lw=1.0)
    ax_c.axhline(coh_thresh, color='r', ls=':', lw=1.0,
                 label=f'γ² = {coh_thresh}')
    ax_c.fill_between(fb, 0, coh, where=coh < coh_thresh, color='r', alpha=0.12)

    label = ('strain transfer amplitude [%]' if which == 'eta'
             else '|H_mid| [-]')
    ax_a.set_ylabel(label)
    if which == 'eta':
        ax_a.axhline(100.0, color='k', lw=0.8, alpha=0.4)
        if cfg.get('eta_pred') is not None:
            ax_a.axhline(cfg['eta_pred'] * 100.0, color='k', ls='--', lw=1.0,
                         alpha=0.6, label='η = 1/(1+Θ) (quasi-static)')
    if logy:
        ax_a.set_yscale('log')
    ...
    ax_c.set_ylabel('coherence γ²')
    ax_c.set_ylim(0, 1.05)
    ax_c.set_xlabel('frequency [Hz]')
    ax_a.set_xlim(0, f_max)
    ax_a.legend(fontsize=8, loc='best')
    ax_c.legend(fontsize=8, loc='lower right')
    ax_a.set_title(f"{cfg.get('label', '?')} — FRF estimator comparison "
                   f"({'strain transfer' if which == 'eta' else 'midpoint'})")
    plt.tight_layout()
    plt.show()
    return fig
...
# ─────────────────────────────────────────────────────────────────────────────
# Extension / compression asymmetry figures (asymmetry.py)
# ─────────────────────────────────────────────────────────────────────────────
def plot_asymmetry_sweep(cfg, f_max=200.0, figsize=(11, 8)):
    """Three-panel asymmetry overview for one dataset (needs asymmetry_sweep).

    (1) η_ext and η_comp vs frequency — half-cycle conditional coupling
        efficiencies; separation of the two curves = asymmetric response.
    (2) peak asymmetry α = (A₊−A₋)/(A₊+A₋) of the cable elongation.
    (3) harmonic-distortion ratios |Y(2f)|/|Y(f)| (even → asymmetric term) and
        |Y(3f)|/|Y(f)| (odd → symmetric nonlinearity) for attribution.
    """
    sw = cfg.get('asym_sweep')
    if sw is None:
        raise ValueError('run asymmetry.asymmetry_sweep(cfg) first')
    m = sw['f'] <= f_max

    fig, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize=figsize, sharex=True)
    ax1.plot(sw['f'][m], sw['eta_ext'][m], 'o-', ms=3, lw=1.0, color='C0',
             label=r'$\eta_{ext}$ (endpoints apart)')
    ax1.plot(sw['f'][m], sw['eta_comp'][m], 's-', ms=3, lw=1.0, color='C3',
             label=r'$\eta_{comp}$ (endpoints together)')
    if cfg.get('eta_pred') is not None:
        ax1.axhline(cfg['eta_pred'], color='k', ls='--', lw=0.9, alpha=0.6,
                    label=r'$\eta = 1/(1+\Theta)$')
    ax1.set_ylabel('η (half-cycle median)')
    ...
    ax3.plot(sw['f'][m], sw['h2_ratio'][m], 'o-', ms=3, lw=1.0, color='C4',
             label='|Y(2f)|/|Y(f)|  (even / asymmetric)')
    ax3.plot(sw['f'][m], sw['h3_ratio'][m], 's-', ms=3, lw=1.0, color='C5',
             label='|Y(3f)|/|Y(f)|  (odd / symmetric)')
    ax3.set_ylabel('harmonic ratio')
...
def plot_asymmetry_overview(datasets, f_min=2.0, f_max=50.0, figsize=(10, 5.5)):
    """Band-mean η_ext vs η_comp for every configuration (one point each).

    Points on the 1:1 line respond symmetrically; departure from the line is
    the extension/compression asymmetry averaged over [f_min, f_max]. Marker
    encoding matches the other overview figures (colour=cable, shape=gap,
    open=Sag).
    """
    ...
    for cfg in datasets:
        ...
        if not m.any():
            continue
        ee = np.nanmean(sw['eta_ext'][m])
        ec = np.nanmean(sw['eta_comp'][m])
        col, marker, fillstyle = dataset_style(cfg)
        ax.plot(ec, ee, marker=marker, ms=8, ls='none', color=col,
                fillstyle=fillstyle, mew=1.2, label=cfg['label'])
        lo = min(lo, ee, ec); hi = max(hi, ee, ec)
    if np.isfinite(lo):
        pad = 0.1 * (hi - lo + 1e-9)
        line = np.array([lo - pad, hi + pad])
        ax.plot(line, line, 'k-', lw=0.8, alpha=0.5, zorder=0)
        ax.set_xlim(*line); ax.set_ylim(*line)
    ax.set_xlabel(r'$\eta_{comp}$ (band mean)')
    ax.set_ylabel(r'$\eta_{ext}$ (band mean)')
    ax.set_title(f'Extension vs compression coupling, {f_min:.0f}–{f_max:.0f} Hz')
