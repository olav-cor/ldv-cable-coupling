...
def spectral_fingerprint(cfg, gap_idx=None):
    """Per-sensor amplitude spectra of v_y, v_z → fingerprints across sensors.

    Returns a dict with:
        freqs    : (N_f,)  frequency axis [Hz]
        spec_y   : (N_f, N_gap)  single-sided |V_y| per gap sensor
        spec_z   : (N_f, N_gap)
        ...
        fp_total : (N_f,)  fp_y_pow + fp_z_pow               (detection signal)
    """
    if gap_idx is None:
        gap_idx = gap_indices(cfg)
    freqs, spec_y = amplitude_spectrum(cfg['vy'][:, gap_idx], cfg['fs'])
    _, spec_z = amplitude_spectrum(cfg['vz'][:, gap_idx], cfg['fs'])
...
def analyze_dataset(cfg, f_min=5.0, f_max=500.0, prominence=0.05,
    ...
    fp = spectral_fingerprint(cfg, gap_idx)
